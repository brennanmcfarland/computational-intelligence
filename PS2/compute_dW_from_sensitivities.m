...
dW_Lminus1_cum = Wji*0;
    ...
    deltas_L = phi_prime_L_vecs .* err_vecs; %FIXED
    ...
    for p=1:P
      deltas_Lminus1(:,p) = (Wkj' * deltas_L(:,p)) .* phi_prime_Lminus1_vecs(:,p); %FIXED
    end
    ...
    dW_Lminus1 = deltas_Lminus1(:,1) * training_patterns(:,1)'; %FIXED
    dW_Lminus1_cum = dW_Lminus1;
    dWL = deltas_L(:,1) * outputs_j(:,1)'; %FIXED
    dWL_cum = dWL;
    for p=2:P
        %layer L synapse sensitivities:
        dWL = deltas_L(:,p) * outputs_j(:,p)'; %FIXED
        dWL_cum = dWL_cum + dWL;
        %layer L-1 synapse sensitivities
       dW_Lminus1 = deltas_Lminus1(:,p) * training_patterns(:,p)'; %FIXED
