...
[dim_outputs,P] =size(training_patterns); %dim of output vec and num training patterns
    ...
    for pattern=2:P
        dW = deltas(:,pattern)*training_patterns(:,pattern)';
        dW_cum = dW_cum + dW;
    end
    ...
    for layer=2:L_layers
        ...
        dW = deltas(:,1) * (all_x_vecs{layer-1}(1))'; % TODO: what is xofl-1ofn equivalent to?
        dW_cum = dW;
        for pattern=2:P
            dW = deltas(:,pattern)* (all_x_vecs{layer-1}(pattern))'; % TODO: fill in as above
            dW_cum = dW_cum + dW;
        end
