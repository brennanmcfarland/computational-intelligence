...
iterations_to_convergence = [0,0];

%iterate over both dimensions of the input to train both x and y
for z = 1:2
  [nnodes_layer2,dummy] = size(targets(z,:)) %number of outputs; this will just be "1"
  ...
  while (should_stop == 0)
      ...
      W1p = W1p_new;
      %FIX THIS FUNCTION...
      [dWL_cum,dW_Lminus1_cum,delta_L_cum,delta_Lminus1_cum] = compute_dW_from_sensitivities(W1p,b1_vec,phi1_code,W21,b2_vec,phi2_code,premapped_training_patterns,targets(z,:));
  ...
  %    [est_dWkj,delta_L_est]= numer_est_Wkj(W1p,b1_vec,W21,b2_vec,premapped_training_patterns,targets(z,:)); %and numerical estimate
  ...
  %     [est_dWji,delta_Lminus1_est]= numer_est_Wji(W1p,b1_vec,W21,b2_vec,premapped_training_patterns,targets(z,:)); %and numerical estimate
  %     est_dWji
  %     delta_Lminus1_cum
  %     delta_Lminus1_est
     ...
     %DEBUG: comment out the above lines from DEBUG to DEBUG when ready
      ...
      [rms,Esqd] = err_eval(W1p,b1_vec,phi1_code,W21,b2_vec,phi2_code,premapped_training_patterns,targets(z,:));
      [rms,Esqd_new] = err_eval(W1p_new,b1_vec_new,phi1_code,W21_new,b2_vec_new,phi2_code,premapped_training_patterns,targets(z,:));
      dE = 0.5*(Esqd_new-Esqd);
      % ME: uncomment to use dynamic lr again
  ...
  %premapped_outputs = compute_premapped_outputs(Wx1,bvec_x1,Wx2,bvec_x2,Wy1,bvec_y1,Wy2,bvec_y2,test_inputs_x,test_inputs_y);
           ...
           hold on
           plot3(training_patterns(1,:),training_patterns(2,:),targets(z,:),'*')
           hold off

          iter1k=iteration;
          iteration
          [rmserr,esqd] = err_eval(W1p,b1_vec,phi1_code,W21,b2_vec,phi2_code,premapped_training_patterns,targets(z,:))
          ...
          iterations_to_convergence(z) = iteration;
      end
